...
function criarNovoParticipante (participante){
    ...
    <tr>
        ...
        <small><i>${participante.email}</i></small>
        </td>
        <td>${dataInscricao}</td>
        <td>${dataCheckIn}</td>
    </tr>
...
function adicionarParticipante(event) {
    ...
    const participanteExiste = participantes.find(
        (p) =>  p.email == participante.email // forma resumida do retorno de uma função
    )
...
function fazerCheckIn(event){
    const resultado = 'Tem certeza que deseja realizar o check-in?'
